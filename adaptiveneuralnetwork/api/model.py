...
class AdaptiveModel(nn.Module):
    ...
    def __init__(self, config: AdaptiveConfig):
        ...
        self.micro_phase_scheduler = MicroPhaseScheduler(
            num_nodes=config.num_nodes,
            device=config.device,
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        self.node_state = self.dynamics(
            self.node_state, node_input, self.phase_scheduler, self.micro_phase_scheduler
        )

        # Update micro-phases for fine-grained monitoring/modulation
        # Use last batch for scheduling decisions
        with torch.no_grad():
            self.micro_phase_scheduler.step(
                major_phases=self.phase_scheduler.node_phases,
                energy_levels=self.node_state.energy[0, :, 0].tolist(),
                activity_levels=self.node_state.activity[0, :, 0].tolist()
            )
    ...
    def reset_state(self) -> None:
        ...
        self.micro_phase_scheduler.reset()
    ...
    def get_metrics(self) -> dict[str, float]:
        ...
        # Add micro-phase stats
        micro_dist = self.micro_phase_scheduler.get_micro_phase_distribution()
        metrics.update({f"micro_{k.lower()}": v / self.config.num_nodes for k, v in micro_dist.items()})

        # Add sleep cycle stats
        sleep_stats = self.micro_phase_scheduler.get_sleep_cycle_stats()
        metrics.update(sleep_stats)
