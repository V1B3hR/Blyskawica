...
class SignalAdapter:
    ...
    def _fetch_local_data(self, source: SignalSource) -> dict[str, Any] | None:
        """Fetch signal data from local system telemetry instead of the network."""
        try:
            ...
            logger.error(f"Local data fetch failed for {source.name}: {e}")
            source.error_count += 1
            return None
    ...
    def fetch_signals(self, source_name: str, params: dict | None = None) -> dict[StateVariable, float]:
        ...
        elif source.signal_type == SignalType.LOCAL_TELEMETRY or source.api_url.startswith("local://"):
            data = self._fetch_local_data(source)
        else:
