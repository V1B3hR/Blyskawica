...
class NonStationaryDataHandler:
    ...
    def detect_distribution_shift(self) -> bool:
        ...
        recent_window = max(1, min(100, len(self.statistics_buffer) // 4))
...
class EpisodicMemory(nn.Module):
    ...
    def sample(
        ...
        importance_weights = self.memory_importance[:available_samples].clone()
        if exclude_task is not None:
            task_mask = self.memory_tasks[:available_samples] != exclude_task
            importance_weights = importance_weights * task_mask.float()
...
class ContinualLearningSystem(nn.Module):
    ...
    def __init__(self, config: ContinualLearningConfig):
        ...
        self.episodic_memory = EpisodicMemory(
            ...
            feature_size=self.output_projection.in_features,
        ...
        self.synaptic_consolidation = SynapticConsolidation(self) if config.synaptic_consolidation else None
        ...
        self.to(self.device)
        logger.info(f"ContinualLearningSystem moved to device: {self.device}")
    ...
    def _build_network(self, base_config: NeuromorphicConfig) -> None:
        ...
        neuron_config = NeuronV3Config(
            ...
            lung_params={"amplify_factor": 20.0}  # Amplify signals for better spiking
        )
        ...
        # Output projection
        feature_dim = self.config.hidden_layers[-1] * 2 if self.config.enable_sparse_coding else self.config.hidden_layers[-1]
        self.output_projection = nn.Linear(feature_dim + self.config.input_size, self.config.output_size)
    ...
    def forward(self, x: torch.Tensor, task_id: int | None = None) -> torch.Tensor:
        ...
        # Final output projection (using skip connection for stability)
        x = x.to(self.device)
        features = features.to(self.device)
        combined_features = torch.cat([features, x], dim=1)
        output = self.output_projection(combined_features)

        # Cache combined_features so callers can retrieve it for episodic memory
        # without needing a second (graph-breaking) forward pass.
        self.last_combined_features = combined_features.detach()
    ...
    def learn_task(
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, (data, target) in enumerate(train_loader):  # noqa: B007
                ...
                with torch.no_grad():
                    ...
                    combined_features = torch.cat([features, data], dim=1)
                    self.episodic_memory.store(combined_features, target, task_id, importance)
                ...
                if task_id > 0 and np.random.random() < replay_ratio:
                    ...
                    memory_features, memory_labels, memory_tasks = self.episodic_memory.sample(
                        replay_samples, exclude_task=task_id
                    )
            ...
            # Enhanced epoch logging (every 10 epochs to reduce verbosity)
        ...
        task_stats['accuracy'] = final_performance
        return task_stats
