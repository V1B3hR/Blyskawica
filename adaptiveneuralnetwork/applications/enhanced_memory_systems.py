...
class DynamicPriorityBuffer:
    ...
    def store(
        ...
        # Dynamically adapt feature dimension if there is a mismatch
        if features.size(1) != self.feature_size:
            self.feature_size = features.size(1)
            self.features = torch.zeros(self.memory_size, self.feature_size, device=features.device, dtype=features.dtype)
