...
class SensoryPreprocessor(nn.Module):
    ...
    def forward(
        ...
        self.input_history = self.input_history.detach()
...
class ModalityProcessor(nn.Module):
    ...
    def __init__(
        ...
        # Calculate input dimension for feature projection
        projection_input_dim = 0
        if self.temporal_encoder is not None:
            projection_input_dim += (feature_size // 2)
            if self.phase_encoder is not None:
                # Based on the cat logic in forward: phase_encoded is sliced to match temporal_patterns size
                projection_input_dim += (feature_size // 2)
        elif self.phase_encoder is not None:
            projection_input_dim = input_size * temporal_config.phase_resolution
        else:
            projection_input_dim = input_size

        # Feature projection
        self.feature_projection = nn.Linear(projection_input_dim, feature_size)
    ...
    def forward(
        ...
        if self.oscillatory_system is not None:
            ...
            # Repeatedly broadcast the oscillators across the feature space
            repeats = (spike_input.size(1) // oscillations.size(1)) + 1
            osc_modulation = oscillations.repeat(1, repeats)[:, :spike_input.size(1)]
            modulated_input = spike_input + osc_modulation * 0.1
        ...
        # Apply sparse coding
        self.sparse_encoder = self.sparse_encoder.to(device)
...
class CrossModalIntegration(nn.Module):
    ...
    def __init__(
        ...
        # Modality projections to harmonize dimensions for cross-attention
        self.modality_projections = nn.ModuleDict()
        for modality, dim in modality_dims.items():
            self.modality_projections[modality] = nn.Linear(dim, integration_dim)
        ...
        if config.enable_cross_modal_binding:
            ...
            self.modality_delays = nn.ParameterDict()
    ...
    def forward(
        ...
        for i, source_modality in enumerate(modality_list):  # noqa: B007
            ...
            for target_modality in modality_list:
                if source_modality != target_modality:
                    target_feat_proj = projected_modalities[target_modality]

                    # Cross-attention between modalities
                    attended, attn_weights = self.cross_attention[source_modality](
                        source_feat_proj.unsqueeze(1),  # Add sequence dimension
                        target_feat_proj.unsqueeze(1),
                        target_feat_proj.unsqueeze(1)
                    )
        ...
        if self.binding_oscillations is not None:
            ...
            binding_signals, binding_info = self.binding_oscillations(
                external_input=combined_activity.unsqueeze(1).repeat(1, self.binding_oscillations.num_oscillators),
                current_time=current_time
            )

            # Modulate features with binding signals
            for modality in attended_features:
                # Repeatedly broadcast binding signals across the feature space
                repeats = (attended_features[modality].size(1) // binding_signals.size(1)) + 1
                binding_mod_broad = binding_signals.repeat(1, repeats)[:, :attended_features[modality].size(1)]
                attended_features[modality] = attended_features[modality] + binding_mod_broad * 0.1
