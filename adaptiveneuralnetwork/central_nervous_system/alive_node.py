"""
[Moduł: Serce Świadomości (AliveLoopNode)]
Reprezentuje biologiczną esencję węzła sieci – żywy organizm cyfrowy, który oddycha, 
śni i odczuwa. Zarządza homeostazą energetyczną, cyklami dobowymi oraz integracją 
głębokiej intuicji kwantowej. 
...
class Memory:
    ...
    def access(self, accessor_id: int) -> Any:
        """Dostęp do treści pamięci z logowaniem audytowym."""
        self.access_count += 1
...
class AliveLoopNode:
    """
    [Rdzeń: Żywy Węzeł]
    Główna klasa zarządzająca życiem pojedynczego węzła w sieci. 
    ...
    def __init__(self, position, velocity, initial_energy=10.0, field_strength=1.0, node_id=0, spatial_dims=None, config=None):
        ...
        self.energy_capacity = float(initial_energy)  # Max energy this node can hold (mirrors Cell in network.py)
    ...
    def update(self, external_activity, internal_stimuli, emotional_trigger=0.0):
        ...
        if hasattr(self, 'dual_rotor_engine') and self.dual_rotor_engine is None and isinstance(external_activity, torch.Tensor) and external_activity.ndim >= 2:
            try:
                from adaptiveneuralnetwork.cognitive_tools.quantum_dual_rotor import DualRotorEngine
                dim = external_activity.shape[-1]
                self.dual_rotor_engine = DualRotorEngine(hidden_dim=dim).to(external_activity.device)
                self._dual_rotor_inner_state = torch.zeros(1, dim, device=external_activity.device)
        ...
        if getattr(self, 'dual_rotor_engine', None) is not None and isinstance(external_activity, torch.Tensor) and external_activity.ndim >= 2:
            try:
                ...
                # Uruchamiamy Falownik Kwantowy przez dławiki!
                self._dual_rotor_inner_state, self._dual_rotor_outer_state, (freq_in, freq_out) = self.dual_rotor_engine(
                    sensory_in, self._dual_rotor_inner_state, self._dual_rotor_outer_state
                )
                # Świadomość bierze się z przefiltrowanego, stabilnego Zewnętrznego Wirnika
                self.activity = self._dual_rotor_outer_state.mean().item()
            except Exception as e:
                logger.warning(f"Błąd fuzji DualRotor: {e}")
                val = external_activity.mean()
                self.activity = float(val.item() if hasattr(val, 'item') else val)
        elif isinstance(external_activity, (torch.Tensor, np.ndarray)):
            # Handle both torch and numpy
            if hasattr(external_activity, 'mean'):
                val = external_activity.mean()
                self.activity = float(val.item() if hasattr(val, 'item') else val)
            else:
                self.activity = 0.0
        else:
            self.activity = float(external_activity)

        # Energy consumption (base 0.01 + activity-driven 0.05)
        drain = 0.01 + (self.activity * 0.05)
        ...
        # Homeostatic energy recovery and Memory Consolidation
        if self.phase == "sleep":
            ...
            # Record significant events during wakefulness
            if self.activity > 0.7:
                self.consolidation.record_event("high_activity", f"Activity spike: {self.activity:.2f}", importance=self.activity)
    ...
    def _process_memory_signal(self, signal: SocialSignal):
        ...
        if hasattr(self, 'epistemic_quarantine'):
            ...
            if not is_valid or anxiety_factor > 1.5:
                ...
                self.epistemic_quarantine._quarantine(knowledge_package, reason)
                self.neurochemistry.trigger_cortisol_spike(0.1) # Skepticism is stressful
                # Debates are now handled periodically/during sleep, not immediately.
                if self.neurochemistry.cortisol > 0.8:
                    logger.warning(f"Node {self.node_id}: High Cortisol detected. Epistemic shield at MAXIMUM.")
                return
    ...
    def _process_query_signal(self, signal: SocialSignal) -> SocialSignal | None:
        ...
        if signal.requires_response:
            # Polymathic Hub Routing
            content_str = str(signal.content).lower()
            poly_keywords = ['gravity', 'velocity', 'quantum', 'molecule', 'chemical', 'reaction', 'coordinates', 'map']
            if hasattr(self, 'polymathic_hub') and any(k in content_str for k in poly_keywords):
                energy_cost, polymath_response = self.polymathic_hub.process_polymathic_signal(content_str, self.energy)
                if self.energy > energy_cost:
                    # Execute polymathic response
                    self.energy -= energy_cost
                    return SocialSignal(
                        content=polymath_response,
                        signal_type="memory",
                        urgency=0.8,
                        source_id=self.node_id
                    )
    ...
    def _process_warning_signal(self, signal: SocialSignal):
        ...
        from adaptiveneuralnetwork.central_nervous_system.spatial_utils import zero_vector
    ...
    def _determine_phase_transition(self) -> None:
        ...
        wm_usage = len(self.working_memory) / self.working_memory.maxlen if self.working_memory.maxlen else 0.0
        if entropy >= 1.0 or noise >= 1.0 or wm_usage >= 1.0:
            self.phase = "sleep"
        ...
        elif self.energy > 20.0 and self.anxiety < 5.0 and self.neurochemistry.serotonin > 0.6:
            self.phase = "inspired"  # High energy, low anxiety, good mood
        elif self.energy > 8.0 and not is_night:
            self.phase = "active"
        else:
    ...
    def handle_attack_detection(self) -> str | None:
    ...
    def process_empathic_interaction(self, user_id: str, audio_features=None, video_features=None, bci_features=None, dt=1.0) -> dict:
        """
        Integracja z MultiUserEmpathicEngine.
        Analizuje rozmówcę (ciepły/chłodny, człowiek/AI) i przygotowuje Błyskawicę
        do odpowiedzi bez przeciążania jej własnych emocji.
        """
        ...
        # Przekazanie sygnałów do MUX (Multi-User Experience)
        engine_response = self._empathic_engine.process_multimodal_interaction(
            user_id=user_id,
        ...
        if bond < 0.8:  # 'Chłodny' rozmówca lub inne AI
            ...
            self.emotional_contagion_sensitivity = 0.8 # Otwiera się na empatię
    ...
    def apply_energy_drain_resistance(self, original_drain: float, attacker_count: int = 1) -> float:
        """Apply resistance to energy drain attacks."""
        base_resistance = getattr(self, 'energy_drain_resistance', 0.7)
        effective_drain = original_drain * (1.0 - base_resistance * 0.7)
        if attacker_count > 1:
            effective_drain = effective_drain * (1.0 + 0.2 * (attacker_count - 1)) / attacker_count
        return min(effective_drain, original_drain)

    def share_energy_directly(self, target_node: 'AliveLoopNode', amount: float) -> float:
        """Share energy directly with another node."""
        trust = self.trust_network.get(target_node.node_id, 0.5)
        if trust < 0.2:
            return 0.0
        to_share = min(amount, self.energy * 0.5)
        self.energy -= to_share
        target_node.energy = target_node.energy + to_share
        self.energy_sharing_history.append({"target": target_node.node_id, "amount": to_share, "type": "shared"})
        target_node.energy_sharing_history.append({"source": self.node_id, "amount": to_share, "type": "received"})
        return to_share

    def request_energy_sharing(self, requester_nodes: list['AliveLoopNode'], energy_needed: float) -> float:
        """Receive requests from other nodes to share energy."""
        total_shared = 0.0
        for node in requester_nodes:
            trust = self.trust_network.get(node.node_id, 0.5)
            if trust < 0.2:
                self.suspicious_events.append((self._time, {
                    "type": "potential_energy_drain_attack",
                    "source": node.node_id,
                    "amount": energy_needed
                }))
                if energy_needed > 5.0:
                    continue
            if self.energy > 1.0:
                to_share = min(energy_needed, self.energy * 0.1)
                self.energy -= to_share
                node.energy = node.energy + to_share
                total_shared += to_share
        return total_shared

    def _detect_signal_jamming(self) -> bool:
        """Detect potential signal jamming."""
        failed_count = sum(1 for sig in self.signal_history if getattr(sig, 'transmission_failed', False))
        return failed_count >= 3

    def _send_with_jamming_resistance(self, signal: Any, receiver: 'AliveLoopNode') -> bool:
        """Send a signal with jamming resistance logic."""
        if signal is None:
            return False
        import random
        jamming_detected = self._detect_signal_jamming()
        if jamming_detected:
            success_prob = 0.65 + (self.signal_redundancy_level * 0.06)
        else:
            success_prob = 0.95
        success = random.random() < success_prob
        if success:
            if hasattr(receiver, 'communication_queue'):
                receiver.communication_queue.append(signal)
            self.signal_history.append(signal)
        return success

    def _record_energy_transaction(self, node_id: int, amount: float, direction: str) -> None:
        """Record an energy transaction."""
        if not hasattr(self, '_energy_transactions'):
            self._energy_transactions = []
        self._energy_transactions.append({
            "node_id": node_id,
            "amount": amount,
            "direction": direction,
            "timestamp": self._time
    ...
    def detect_long_term_trust_manipulation(self) -> list[int]:
        """Detect long term trust manipulation attempts."""
        if not hasattr(self, '_energy_transactions'):
            return []
        suspicious = []
        from collections import defaultdict
        node_txs = defaultdict(list)
        for tx in self._energy_transactions:
            node_txs[tx["node_id"]].append(tx)
        for node_id, txs in node_txs.items():
            if len(txs) >= 5:
                amounts = [t["amount"] for t in txs]
                times = [t["timestamp"] for t in txs]
                time_diffs = [times[i] - times[i-1] for i in range(1, len(times))]
                if np.var(amounts) < 0.05 and np.mean(time_diffs) < 5.0:
                    suspicious.append(node_id)
        return suspicious

    def _calculate_trust_adjustment_factor(self, node_id: int) -> float:
        """Calculate trust adjustment factor for trust updates."""
        if node_id in self.detect_long_term_trust_manipulation():
            return 0.2
        if not hasattr(self, '_energy_transactions'):
            return 0.5
        txs = [t for t in self._energy_transactions if t["node_id"] == node_id]
        if len(txs) < 3:
            return 0.5
        return 1.0

    def _update_trust_after_communication(self, target_node: 'AliveLoopNode', signal_type: str) -> None:
        """Update trust level after communication."""
        factor = self._calculate_trust_adjustment_factor(target_node.node_id)
        current_trust = self.trust_network.get(target_node.node_id, 0.5)
        if signal_type == "resource":
            delta = 0.05 * factor
        else:
            delta = 0.01 * factor
        self.trust_network[target_node.node_id] = min(1.0, max(0.0, current_trust + delta))
        self._update_trust_attribute()
    ...
    def _predict_future_energy_needs(self) -> float:
        ...
        count = 0
        for m in self.memory:
            if m.memory_type == "usage" and isinstance(m.content, dict) and "energy" in m.content:
                recent_drain += abs(m.content["energy"])
                count += 1
        if count == 0:
            return 5.0
        avg_drain = recent_drain / count
        return avg_drain * 10.0

    def _optimize_energy_usage(self) -> None:
        """Optimize energy usage under constraints."""
        if self.energy < self.emergency_energy_threshold * self.energy_capacity:
            self.max_communications_per_step = 2
        else:
            self.max_communications_per_step = 5

    def _adapt_to_environment(self) -> None:
        """Adapt node parameters to recent environment stressors."""
        stress_count = sum(1 for m in self.memory if m.memory_type == "environmental" and m.emotional_valence < -0.5)
        threat_count = sum(1 for m in self.memory if m.memory_type == "threat")
        if stress_count > 0:
            self.energy_drain_resistance = min(0.95, self.energy_drain_resistance + 0.05 * stress_count)
            self.anxiety = min(5.0, self.anxiety + 0.5 * stress_count)
        if threat_count > 0:
            self.attack_detection_threshold = max(1, self.attack_detection_threshold - threat_count)
            self.signal_redundancy_level = min(5, self.signal_redundancy_level + threat_count)

    @property
    def energy(self) -> float:
        """Get the current energy level of the node."""
        return getattr(self, '_energy', 0.0)

    @energy.setter
    def energy(self, value: float) -> None:
        """Set the current energy level, applying adaptive drain resistance if energy is low."""
        if hasattr(self, '_energy') and value < self._energy:
            drain = self._energy - value
            if self._energy < 0.5:
                # Apply drain resistance under low energy conditions (e.g. low energy stress testing)
                resistance = getattr(self, 'energy_drain_resistance', 0.7)
                drain *= (1.0 - resistance * 0.6)
                value = self._energy - drain
        self._energy = max(0.0, float(value))
