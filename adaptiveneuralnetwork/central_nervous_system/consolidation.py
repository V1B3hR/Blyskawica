...
class ConsolidationType(Enum):
    ...
    NARRATIVE = "narrative"         # Episodic to autobiography synthesis
...
class PhaseBasedConsolidation(ConsolidationMechanism):
    ...
    def consolidate(
        ...
        **kwargs
        ...
        sleep_nodes = phase_scheduler.node_phases == Phase.SLEEP.value
        ...
        if light_nodes.any():
            activity[light_nodes] *= (1 - self.config["memory_decay"])
            results["modifications"].append(f"LIGHT: Downscaled {light_nodes.sum()} nodes")

        # 2. DEEP SLEEP: Stability Boost & Semantic Crystallization
        deep_nodes = sleep_nodes & (sub_phases == SubPhase.DEEP.value)
        if deep_nodes.any():
            # Boost energy for important nodes (high activity before sleep)
            important_nodes = activity > 0.5
            consolidation_nodes = deep_nodes & important_nodes
            if consolidation_nodes.any():
                energy[consolidation_nodes] *= self.config["stability_boost"]
                results["modifications"].append(f"DEEP: Stabilized {consolidation_nodes.sum()} nodes")

        # 3. REM SLEEP: Dreaming (Randomized Pattern Replay)
        rem_nodes = sleep_nodes & (sub_phases == SubPhase.REM.value)
        if rem_nodes.any():
            # In REM, we inject small random fluctuations to simulate dream replay
            # and test new representational combinations
            dream_noise = torch.randn_like(activity[rem_nodes]) * 0.1
            activity[rem_nodes] = torch.clamp(activity[rem_nodes] + dream_noise, 0.0, 1.0)
            results["modifications"].append(f"REM: Dream Replay for {rem_nodes.sum()} nodes")
        ...
        results["metrics"] = {
            ...
            "sub_phases": {
                "light": light_nodes.sum().item(),
                "deep": deep_nodes.sum().item(),
                "rem": rem_nodes.sum().item()
            }
        }
...
class SynapticConsolidation(ConsolidationMechanism):
    ...
    def _estimate_fisher_information(self, dataloader: torch.utils.data.DataLoader):
        """Estimate Fisher Information for the current task."""
        self.model.eval()
        sample_count = 0

        for batch in dataloader:
            # Handle multimodal loaders
            if len(batch) >= 2:
            ...
            for i in range(batch_size):
                ...
                with torch.no_grad():
                    # 1. Reset explicit neuron states
                    if hasattr(self.model, 'network') and hasattr(self.model.network, 'reset_state'):
                    ...
                    # 3. Clear gradients and cache
                    self.model.zero_grad(set_to_none=True)
                    if torch.cuda.is_available():
                        torch.cuda.empty_cache()

                # 2. Focused observation (Individual forward pass)
                # Ensure inputs are detached from any previous history
                sample_inputs = [x.detach().clone() if torch.is_tensor(x) else x for x in sample_inputs]
                ...
                with torch.no_grad():
                    for name, param in self.model.named_parameters():
                        if param.requires_grad and param.grad is not None:
                            # Isolate the gradient contribution completely
                            grad_sq = param.grad.detach().pow(2)
                            self.fisher_information[name] = (self.fisher_information[name] + grad_sq).detach()
                            # Wipe the gradient immediately
                            param.grad = None

                sample_count += 1
                # 5. Step back to the safety line (Graph is freed here)

        # Normalize Fisher information
        with torch.no_grad():
            if sample_count > 0:
                for name in self.fisher_information:
                    self.fisher_information[name] = (self.fisher_information[name] / sample_count).detach()
...
class NarrativeConsolidation(ConsolidationMechanism):
    ...
    def __init__(self, narrative_engine: NarrativeEngine, **config):
        ...
        default_config.update(config)
        super().__init__("narrative_consolidation", ConsolidationType.NARRATIVE, **default_config)
        self.narrative_engine = narrative_engine

    def consolidate(self, episodic_observations: torch.Tensor, **kwargs) -> dict[str, Any]:
        """Synthesize current episodes into the long-term narrative."""
        results = {"modifications": [], "metrics": {}}
        if not self.is_active:
            return results
