...
class PopulationLayer(nn.Module):
    ...
    def forward(
        ...
        self.homeo_bias = self.homeo_bias.detach()

        batch_size = external_input.size(0)
        population_spikes = torch.zeros(batch_size, self.population_size, device=device)
        neuron_states = []

        # 1. Axonal Delays with Dimensional Armor
        if hasattr(self, 'synaptic_delays'):
            ...
            delayed_input = external_input

        # 2. Dimensional Armor for external input vs population size
        if delayed_input.size(-1) != self.population_size:
            if delayed_input.size(-1) < self.population_size:
                padding_shape = list(delayed_input.shape)
                padding_shape[-1] = self.population_size - delayed_input.size(-1)
                padding = torch.zeros(*padding_shape, device=delayed_input.device)
                delayed_input = torch.cat([delayed_input, padding], dim=-1)
            else:
                delayed_input = delayed_input[..., :self.population_size]

        # 3. Process lateral & recurrent feedback
        lateral_input = torch.zeros_like(delayed_input)
        if self.lateral_inhibition:
            # Ensure lateral components are on the correct device
        ...
        if hasattr(self, 'recurrent_weights'):
            # Reuse the already-synced activity
            prev_act = activity.unsqueeze(0).expand(batch_size, -1)
            ...
            if prev_act.size(-1) != rec_weights.size(1):
                 if prev_act.size(-1) < rec_weights.size(1):
                     padding = torch.zeros(*prev_act.shape[:-1], rec_weights.size(1) - prev_act.size(-1), device=prev_act.device)
        ...
        if modulatory_config:
            ...
            if metabolic_gate < 0.9:
                ...
                gate_mask = (torch.rand_like(total_input) < metabolic_gate).float()
        ...
        for i, neuron in enumerate(self.neurons):
            # Extract signal for this specific neuron
            # Handle 3D [batch, nodes, features] or 2D [batch, features]
            if total_input.dim() == 3:
                # If 3D, we assume node-wise signal is at dim 1
                # But if we have 32 nodes and auditing a 128-layer...
                # We use the index modulo the available nodes to prevent overflow
                node_idx = i % total_input.size(1)
                sig = total_input[:, node_idx, :]
            else:
                sig = total_input[:, i:i+1]

            if self.neuron_type == "multi_compartment":
                # Multi-compartment expects [batch, 4] for dendrites, [batch, 1] for soma
                # We'll armor it: take first 4 features or pad
                if sig.size(-1) < 4:
                    dendrite_input = torch.cat([sig, torch.zeros(batch_size, 4 - sig.size(-1), device=sig.device)], dim=-1)
                else:
                    dendrite_input = sig[:, :4]
                soma_input = sig[:, 0:1]
        ...
        # Update population activity tracking (Detach to prevent graph retention)
        current_activity = population_spikes.mean(dim=0).detach()  # Average across batch
...
class RealisticDelays(nn.Module):
    ...
    def forward(
        ...
        source_size = input_spikes.size(1)

        # 1. ENSURE INPUT SHAPE INTEGRITY & Buffer Resizing
        if source_size != self.source_size:
            logger.warning(f"Resizing SynapticDelayLine buffer from {self.spike_buffer.size(1)} to {source_size}")
            # Reset buffer with new size
            self.spike_buffer = torch.zeros(self.spike_buffer.size(0), source_size, device=device)
            self.source_size = source_size
            # Re-generate delays if source size changed significantly
            if self.delays.size(0) != source_size:
                 self.delays = self._generate_delays().to(device)

        # 2. Update buffer with current spikes (Detach to prevent graph retention)
        current_idx = self.buffer_index.item() % self.spike_buffer.size(0)
        self.spike_buffer[current_idx] = input_spikes.mean(dim=0).detach()

        # 3. Vectorized implementation of synaptic delays
        ...
        if delayed_spikes_base.size(0) < self.target_size:
            ...
            delayed_spikes_base = torch.cat([delayed_spikes_base, padding])
        elif delayed_spikes_base.size(0) > self.target_size:
            delayed_spikes_base = delayed_spikes_base[:self.target_size]

        delayed_spikes = delayed_spikes_base.unsqueeze(0).expand(batch_size, -1)
...
class DynamicConnectivity(nn.Module):
    ...
    def forward(
        ...
        self.correlation_history = self.correlation_history.detach()
        self.connection_lifetimes = self.connection_lifetimes.detach()
        ...
        effective_weights = self.synaptic_weights.to(device) * self.connectivity_mask.to(device)
...
class HierarchicalNetwork(nn.Module):
    ...
    def forward(
        ...
        device = params[0].device if params else input_spikes.device
        ...
        self.layer_activities = torch.stack([output.mean() for output in layer_outputs]).to(device)
        ...
        if activity_correlations:
            ...
            self.network_coherence = torch.tensor(1.0, device=self.device)
