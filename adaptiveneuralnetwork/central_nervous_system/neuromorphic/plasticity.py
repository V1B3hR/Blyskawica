...
class STDPSynapse(nn.Module):
    ...
    def forward(
        ...
        # Expand/reset traces for batch if size changed
        if self.pre_trace.dim() == 1 or self.pre_trace.size(0) != batch_size:
            self.pre_trace = torch.zeros(batch_size, self.pre_size, device=pre_spikes.device)
            self.post_trace = torch.zeros(batch_size, self.post_size, device=post_spikes.device)
            self.last_pre_spike = torch.full((batch_size, self.pre_size), -float('inf'), device=pre_spikes.device)
            self.last_post_spike = torch.full((batch_size, self.post_size), -float('inf'), device=post_spikes.device)
    ...
    def _calculate_stdp_updates(
        ...
        """Calculate STDP weight updates based on temporal pairings."""
        batch_size = pre_spikes.size(0)
        weight_changes = torch.zeros(batch_size, self.pre_size, self.post_size, device=pre_spikes.device)

        # Iterate over batch
        for b in range(batch_size):
            pre_indices = torch.where(pre_spikes[b] > 0)[0]
            post_indices = torch.where(post_spikes[b] > 0)[0]

            # 1. LTP: If a post-neuron spikes now, check all PREVIOUS pre-spikes
            for post_idx in post_indices:
                ...
                if ltp_mask.any():
                    ltp_amount = self.config.a_plus * torch.exp(-dt_ltp[ltp_mask] / self.config.tau_plus)
                    weight_changes[b, ltp_mask, post_idx] += ltp_amount

            # 2. LTD: If a pre-neuron spikes now, check all PREVIOUS post-spikes
            for pre_idx in pre_indices:
                ...
                if ltd_mask.any():
                    ...
                    weight_changes[b, pre_idx, ltd_mask] += ltd_amount
...
class MetaplasticitySynapse(nn.Module):
    ...
    def __init__(
        ...
        # Synaptic consolidation (Phase 7.3)
        self.register_buffer('weight_tags', torch.zeros(pre_size, post_size)) # Tracks importance
        self.register_buffer('protection_factor', torch.ones(pre_size, post_size))
    ...
    def forward(
        ...
        self.weight_tags = self.weight_tags.detach()
        self.protection_factor = self.protection_factor.detach()

        # Expand/reset metaplastic variables for batch
        if self.theta.dim() == 1 or self.theta.size(0) != batch_size:
            self.theta = torch.full((batch_size, self.post_size), self.meta_config.theta_plus, device=post_spikes.device)
            self.x = torch.zeros(batch_size, self.post_size, device=post_spikes.device)
            self.activity_history = torch.zeros(batch_size, self.post_size, 100, device=post_spikes.device)

        # Update activity variable (wrapped dt in tensor to avoid TypeError)
        self.x *= torch.exp(torch.tensor(-dt / self.meta_config.tau_x))
        self.x += post_spikes
        ...
        # Update weight tags (consolidation)
        if hasattr(self, 'stdp_synapse') and learning:
            with torch.no_grad():
        ...
        if learning:
            ...
            # Modulate weight changes [pre, post] * [post] -> [pre, post]
            modulated_changes = base_weight_changes * batch_activity_factor

            # Apply modulated weight changes with protection
            # protection_factor scales down the updates for consolidated weights
            new_weights = self.stdp_synapse.weights + modulated_changes * self.protection_factor
...
class HomeostaticScaling(nn.Module):
    ...
    def forward(
        ...
        # Detach persistent buffers from previous batches
        self.scaling_factors = self.scaling_factors.detach()
        self.spike_counts = self.spike_counts.detach()

        # Expand/reset buffers for batch processing
        if self.spike_counts.dim() == 1 or self.spike_counts.size(0) != batch_size:
            self.spike_counts = torch.zeros(batch_size, self.num_neurons, device=post_spikes.device)
