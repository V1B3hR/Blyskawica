...
class TemporalPatternEncoder(nn.Module):
    ...
    def forward(
        ...
        correlation_kernels = self.correlation_kernels.to(device)
        pattern_weights = self.pattern_weights.to(device)

        # Update spike history
        hist_idx = self.history_index.item() % spike_history.size(0)
        spike_history[hist_idx] = input_spikes[0]  # Use first batch for simplicity
        self.spike_history.copy_(spike_history)
        self.time_history[hist_idx] = current_time
        self.history_index += 1

        # Extract recent spike patterns
        pattern_activations = torch.zeros(batch_size, self.pattern_size, device=device)

        # Simple pattern matching based on recent activity
        recent_window = min(self.config.max_pattern_length, spike_history.size(0))
        recent_spikes = spike_history[-recent_window:]  # [window, input_size]

        # Calculate pattern similarities using correlation kernels
        for pattern_idx in range(self.pattern_size):
            pattern_template = pattern_templates[pattern_idx, :recent_window]

            # Calculate correlation across different temporal kernels
            correlations = []
            for kernel_idx in range(correlation_kernels.size(0)):
                kernel = correlation_kernels[kernel_idx, :recent_window]
        ...
        weighted_patterns = torch.matmul(input_spikes, pattern_weights.t())
...
class PhaseEncoder(nn.Module):
    ...
    def forward(
        ...
        self.reference_oscillator = (self.reference_oscillator.to(device) + omega * dt) % (2 * math.pi)
        ...
        for i in range(self.input_size):
            if spike_mask[i]:
                ...
                phase_bins[i, phase_bin] += 1.0
                self.last_spike_phases[i] = current_phase

        # Decay phase bins
        phase_bins *= 0.95
        self.phase_bins.copy_(phase_bins)

        # Generate phase-encoded output
        phase_encoded = torch.zeros(batch_size, self.input_size * self.config.phase_resolution, device=device)

        # Flatten phase bins for output
        for b in range(batch_size):
            phase_encoded[b] = phase_bins.flatten()
        ...
        phase_info = {
            ...
            'phase_bins': phase_bins.clone(),
            'reference_phase': self.reference_oscillator.to(device).clone(),
            'last_spike_phases': self.last_spike_phases.to(device).clone(),
...
class OscillatoryDynamics(nn.Module):
    ...
    def forward(
        ...
        self.oscillator_outputs = self.oscillator_outputs.detach()
        batch_size = 1
        if external_input is not None:
            batch_size = external_input.size(0)
            external_input = external_input[0]  # Use first batch
        else:
            external_input = torch.zeros(self.num_oscillators, device=device)

        # Ensure all internal state is on the correct device
        phases = self.phases.to(device)
        amplitudes = self.amplitudes.to(device)
        coupling_matrix = self.coupling_matrix.to(device)

        # Update phases based on natural frequencies
        for i, freq in enumerate(self.frequencies):
            omega = 2 * math.pi * freq
            phases[i] = (phases[i] + omega * dt) % (2 * math.pi)

        # Calculate coupling effects
        coupling_effects = torch.zeros(self.num_oscillators, device=device)
        for i in range(self.num_oscillators):
            for j in range(self.num_oscillators):
                if i != j:
                    # Phase difference coupling
                    phase_diff = phases[j] - phases[i]
                    coupling_force = coupling_matrix[i, j] * torch.sin(phase_diff)
        ...
        amplitudes = amplitudes * (1 - amplitude_decay * dt) + external_input * dt
        amplitudes = torch.clamp(amplitudes, 0.1, 2.0)  # Bounds
        self.amplitudes.copy_(amplitudes)

        # Generate oscillator outputs
        osc_outputs = torch.zeros(self.num_oscillators, device=device)
        for i in range(self.num_oscillators):
            osc_outputs[i] = amplitudes[i] * torch.sin(phases[i])
        self.oscillator_outputs.copy_(osc_outputs)

        # Cross-frequency phase coupling (phase-amplitude coupling)
        pac_outputs = torch.zeros(self.num_oscillators, device=device)
        for i in range(self.num_oscillators - 1):
            # Higher frequency amplitude modulated by lower frequency phase
            low_freq_phase = phases[i]
            high_freq_amp = amplitudes[i + 1]
        ...
        # Combine outputs
        combined_output = osc_outputs + pac_outputs
...
class SparseDistributedRepresentation(nn.Module):
    ...
    def forward(
        ...
        # Forward encoding
        raw_activations = torch.matmul(input_activation, self.encoding_weights.to(device))
        ...
        adaptive_thresholds = self.adaptive_thresholds.to(device)
        ...
        for _ in range(num_iterations):
            ...
            inhibition = torch.matmul(current_activations,
                                    lateral_weights * inhibition_mask)

            # Update activations with inhibition
            current_activations = raw_activations - inhibition

            # Apply adaptive thresholds
            current_activations = torch.relu(current_activations - adaptive_thresholds)
        ...
        sparse_representation = torch.zeros_like(current_activations, device=device)
        for b in range(batch_size):
...
class VisualSpikeEncoder(nn.Module):
    """
    Translates visual inputs into spike timing patterns.
    Supports Latency-to-Spike (LTS) and Poisson intensity encoding.
    """

    def __init__(
        self,
        input_shape: tuple[int, int, int],
        encoding_type: str = "latency",
        time_window: float = 0.1,  # 100ms window
        max_rate: float = 100.0    # 100 Hz max
    ):
        super().__init__()
        self.input_shape = input_shape
        self.flat_input_dim = input_shape[0] * input_shape[1] * input_shape[2]
    ...
    def forward(
        ...
        """
        Convert pixel activations into a spike mask [batch, flat_input_dim].
