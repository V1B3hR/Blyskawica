...
class LiverFilter(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.method == 'gaussian':
            ...
            x_padded = torch.nn.functional.pad(x, (self.kernel_size // 2, self.kernel_size // 2), mode='constant')
...
class AdaptiveThresholdNeuron(nn.Module):
    ...
    def forward(
        ...
        if self.v_mem.dim() == 0 or self.v_mem.size(0) != batch_size:
            # Note: In a production stateful system, we'd handle resizing more carefully
            # but here we initialize fresh states for the new batch dimension.
            self.v_mem = torch.full((batch_size,), self.config.base_config.v_rest, device=device)
            self.threshold = torch.full((batch_size,), self.config.base_config.v_threshold, device=device)
            self.spike_count = torch.zeros(batch_size, device=device)
            self.time_window_start = torch.full((batch_size,), current_time, device=device)
            self.last_spike_time = torch.full((batch_size,), -1e9, device=device)
        ...
        dv_dt = (input_current.squeeze(-1) + leak_current) / tau_mem
...
class BurstingNeuron(nn.Module):
    ...
    def forward(
        ...
        if self.v_mem.dim() == 0 or self.v_mem.size(0) != batch_size:
            device = input_current.device
            self.v_mem = torch.full((batch_size,), self.config.base_config.v_rest, device=device)
            self.ahp_current = torch.zeros(batch_size, device=device)
            self.burst_threshold = torch.full((batch_size,), self.config.base_config.v_threshold * 0.8, device=device)
            self.in_burst = torch.zeros(batch_size, dtype=torch.bool, device=device)
            self.burst_spike_count = torch.zeros(batch_size, device=device)
            self.last_spike_time = torch.full((batch_size,), -1e9, device=device)
