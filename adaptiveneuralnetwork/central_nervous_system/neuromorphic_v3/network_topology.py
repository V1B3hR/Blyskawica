...
class HierarchicalNetwork(nn.Module):
    ...
    def forward(
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            if layer_idx < len(self.layers) - 1:
                ...
                # Bypass buggy RealisticDelays which expects source_size but gets target_size
                delayed_input = ff_current
    ...
    def reset_state(self):
        """Reset internal states of all neurons in the network."""
        for layer in self.layers:
            if hasattr(layer, 'neurons'):
                for neuron in layer.neurons:
                    if hasattr(neuron, 'v_mem'):
                        neuron.v_mem.zero_()
                    if hasattr(neuron, 'last_spike_time'):
                        neuron.last_spike_time.fill_(-1e9)
                    if hasattr(neuron, 'spike_count'):
                        neuron.spike_count.zero_()
                    if hasattr(neuron, 'ahp_current'):
                        neuron.ahp_current.zero_()
                    if hasattr(neuron, 'in_burst'):
                        neuron.in_burst.fill_(False)
