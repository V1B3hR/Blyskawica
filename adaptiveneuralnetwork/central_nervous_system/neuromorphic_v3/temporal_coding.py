    ...
    def forward(
        ...
        if curr_size != self.input_size or self.spike_history.size(1) != curr_size:
            ...
            self.spike_history = torch.zeros(
                history_len, curr_size, device=input_spikes.device
            )
            self.history_index.zero_()
            self.pattern_weights = nn.Parameter(
                torch.randn(self.pattern_size, curr_size, device=input_spikes.device)
            )
            self.pattern_templates = torch.randn(
                self.pattern_size, self.config.max_pattern_length, curr_size, device=input_spikes.device
            )

        # Update spike history - use detach to prevent graph leaks between batches
        hist_idx = self.history_index.item() % self.spike_history.size(0)
        self.spike_history[hist_idx] = input_spikes[0].detach()  # Use first batch for simplicity
...
class SparseDistributedRepresentation(nn.Module):
    ...
    def forward(
        ...
        # Build sparse representation using a differentiable mask instead of
        # in-place index assignment, which breaks autograd graph tracking.
        mask = torch.zeros_like(current_activations)
        for b in range(batch_size):
            # Find top-k activations
            _, top_indices = torch.topk(current_activations[b], target_active)
            mask[b, top_indices] = 1.0
        sparse_representation = current_activations * mask

        if adapt_sparsity:
            # All state updates are homeostatic/bookkeeping — no gradients needed.
            # Keeping grad history in these buffers would cause "backward through
            # the graph a second time" errors in subsequent batches.
            with torch.no_grad():
                # Update activity history
                current_activity = sparse_representation.detach().mean(dim=0)
                ...
                new_thresholds = self.adaptive_thresholds + threshold_update
                new_thresholds = torch.clamp(new_thresholds, 0.01, 2.0)
                # Write back using in-place copy to keep the registered buffer valid
                self.adaptive_thresholds.copy_(new_thresholds)
...
class VisualSpikeEncoder(nn.Module):
    """
    Encoder for visual information using intensity-to-latency coding.
    ...
    def forward(
        ...
        """
        Convert visual input to spike latencies.
        
        Args:
            visual_input: Tensor of shape (B, C, H, W) or (B, N)
            current_time: Current simulation time
...
class AudioSpikeEncoder(nn.Module):
    """
    Encoder for audio information using frequency-to-phase coding.
    ...
    def forward(
        self,
        audio_spectrum: torch.Tensor,
        current_time: float = 0.0
    ) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        """
        Convert audio spectrum to spikes.
        
        Args:
            audio_spectrum: Tensor of shape (B, num_frequency_bins)
            current_time: Current simulation time
