...
class RobustnessValidator:
    ...
    def _test_trust_scenario(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        for i in range(10):
            ...
            trust_level = max(0.0, min(1.0, 0.5 + offset + np.random.normal(0, params["trust_variance"] * 0.5)))
            ...
            if np.random.random() < change_frequency:
                ...
                if new_environment != current_environment:
                    ...
                    if (new_environment == "resource_poor" and node.phase == "sleep") or \
                       (new_environment == "resource_rich" and node.phase == "active") or \
                       (new_environment in ["stable", "chaotic"]):
                        adaptation_successes += 1

                    current_environment = new_environment

        adaptation_rate = adaptation_successes / adaptation_attempts if adaptation_attempts > 0 else 1.0
