...
class SafeCompiledModel(nn.Module):
    """
    Wrapper around a compiled model that catches errors during lazy compilation
    (which happens on the first forward pass) and gracefully falls back to eager mode.
    """
    def __init__(self, base_model: nn.Module, compiled_model: nn.Module):
        super().__init__()
        # Avoid registering them as submodules to prevent duplicate parameter/buffer/state_dict references
    ...
    def forward(self, *args, **kwargs):
        ...
        try:
            ...
            print(f"Compilation execution failed, falling back to eager: {e}")
...
def try_compile(model: nn.Module, mode: str = "default") -> nn.Module:
    ...
    try:
        ...
        return SafeCompiledModel(model, compiled_model)
    except Exception as e:
