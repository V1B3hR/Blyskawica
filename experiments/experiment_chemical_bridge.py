...
def generate_triple_modal_data(domain="chemistry", num_samples=50):
    ...
    if domain == "biology": labels = torch.zeros(num_samples, dtype=torch.long)
    elif domain == "physics": labels = torch.ones(num_samples, dtype=torch.long)
    else: labels = torch.full((num_samples,), 2, dtype=torch.long)
