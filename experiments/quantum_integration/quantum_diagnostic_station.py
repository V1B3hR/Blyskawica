...
def run_diagnostic():
    ...
    print("  TEST 1: Zimny Rozruch (Sanity Check)")
    ...
    with torch.no_grad():
        ...
        v_loss = criterion(v_outputs, v_labels)  # noqa: F841
        v_preds = torch.argmax(v_outputs, dim=1)
        v_acc = (v_preds == v_labels).float().mean()
    print("    - Zbiór testowy: 1000 nowych rekordów CERN")
    ...
    print("    - Zakłócenia: 20% szumu w danych wejściowych")
