...
class MemoryConsolidator:
    ...
    def __init__(self, memory_store: AgentMemoryStore, config: dict[str, Any] | None = None):
                ...
                try:
                    ...
                    with sqlite3.connect(self.consolidation_db) as conn:
                        ...
                        conn.execute("""
                            ...
                            VALUES (?, ?, ?, ?, ?)
                        """, (cluster_id, json.dumps(memory_ids), cluster_summary, now, now))  # noqa: W291
                        conn.commit()
                        ...
                        with sqlite3.connect(self.consolidation_db) as conn:
                            ...
                            for mem_id in [mem1['id'], mem2['id']]:
                                conn.execute("""
                                    ...
                                    WHERE memory_id = ?
                                """, (datetime.now().isoformat(), mem_id))  # noqa: W291
    ...
    def _check_memory_conflict(self, mem1: dict[str, Any], mem2: dict[str, Any]) -> dict | None:
    ...
    def _calculate_content_similarity(self, content1: str, content2: str) -> float:
        ...
        intersection = len(words1 & words2)  # noqa: F841
        union = len(words1 | words2)  # noqa: F841
    ...
    def _get_relevant_conflicts(self, memory_ids: list[int]) -> list[dict]:
        ...
        try:
            with sqlite3.connect(self.consolidation_db) as conn:
                ...
                """, memory_ids + memory_ids)  # noqa: W291
    ...
    def resolve_semantic_conflict(self, conflict_id: str, resolution_method: str,
                                 winning_memory_id: int | None = None) -> bool:
        """
            ...
            with sqlite3.connect(self.consolidation_db) as conn:
                ...
                conn.execute("""
                    ...
                    SET resolution_status = 'resolved', 
                        ...
                        resolution_method = ?
                    WHERE conflict_id = ?
                """, (datetime.now().isoformat(), resolution_method, conflict_id))  # noqa: W291

                # Get conflict details
                cursor = conn.execute("""
                    SELECT memory_id1, memory_id2 FROM semantic_conflicts 
                    WHERE conflict_id = ?
                """, (conflict_id,))  # noqa: W291
                    ...
                    if winning_memory_id and winning_memory_id in [mem_id1, mem_id2]:
                        ...
                        conn.execute("""
                            ...
                            SET consolidation_priority = consolidation_priority * 0.5,
                                updated_at = ?
                            WHERE memory_id = ?
                        """, (datetime.now().isoformat(), losing_id))  # noqa: W291
