...
from .human_ai_collaboration import CollaborativeDecision
...
class CollaborationPerformanceMonitor:
    ...
    def get_real_time_dashboard(self, dashboard_id: str | None = None) -> dict[str, Any]:
        """
    ...
    def _get_recent_metric_value(self, metric: PerformanceMetric,
                               window_seconds: int = 300) -> float | None:
    ...
    def _get_metric_history(self, metric: PerformanceMetric, start_time: float,
                          user_id: str | None = None) -> list[MetricDataPoint]:
    ...
    def _calculate_user_percentile(self, user_id: str, metric: PerformanceMetric) -> float:
        ...
        all_user_averages = []
        for uid, user_data in self.user_metrics.items():  # noqa: B007
            if user_data[metric]:
