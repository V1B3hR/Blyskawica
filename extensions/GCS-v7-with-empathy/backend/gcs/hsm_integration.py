...
class HSMSecureStorage:
    ...
    def retrieve_key(self, key_id: str) -> tuple[HSMOperationResult, bytes | None]:
        """
    ...
    def _retrieve_pkcs11_key(self, key_id: str, metadata: HSMKeyMetadata) -> bytes | None:
    ...
    def _retrieve_software_key(self, key_id: str, metadata: HSMKeyMetadata) -> bytes | None:
    ...
    def hsm_encrypt_data(self,
                        key_id: str,
                        plaintext: bytes,
                        additional_data: bytes | None = None) -> tuple[HSMOperationResult, bytes | None]:
        """
    ...
    def _software_encrypt(self, key_data: bytes, plaintext: bytes, additional_data: bytes | None) -> bytes:
    ...
    def _pkcs11_encrypt(self, key_handle_bytes: bytes, plaintext: bytes, additional_data: bytes | None) -> bytes:
    ...
    def _software_decrypt(self, key_data: bytes, ciphertext: bytes, additional_data: bytes | None) -> bytes:
    ...
    def _pkcs11_decrypt(self, key_handle_bytes: bytes, ciphertext: bytes, additional_data: bytes | None) -> bytes:
    ...
    def get_key_metadata(self, key_id: str) -> HSMKeyMetadata | None:
        """Get metadata for a key"""
        return self.key_metadata.get(key_id)

    def list_keys(self, key_type: HSMKeyType | None = None) -> list[HSMKeyMetadata]:
...
def get_hsm_storage(hsm_type: HSMType = HSMType.SOFTWARE_FALLBACK,
                   hsm_config: dict[str, Any] | None = None) -> HSMSecureStorage:
