...
try:
    ...
    from gcs.CognitiveRCD import Action, ActionType, CognitiveRCD, Intent, SafetyLevel  # noqa: F401
    from gcs.DuetMindAgent import DuetMindAgent, DuetMindSystem, StyleVector  # noqa: F401

    # Import test classes for direct execution
    from gcs.tests.test_3ngin3_foundational import (
        TestCognitiveRCD,
        TestDuetMindAgent,
        TestMetaControllerEvaluation,  # noqa: F401
        TestThreeDimensionalHRO,
    )
    from gcs.tests.test_3ngin3_intelligence import (
        ...
        TestMetamorphicTesting,
    )
    from gcs.tests.test_3ngin3_performance import (
        TestConcurrencyThreadSafety,  # noqa: F401
        TestGracefulDegradation,
        TestResourceMonitoring,
    )
    from gcs.ThreeDimensionalHRO import (
        ComputeBackend,  # noqa: F401
        OptimizationStrategy,  # noqa: F401
        ReasoningMode,  # noqa: F401
        ThreeDimensionalHRO,
    )
    ...
    def run_intelligence_tests(self) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                print("     ✓ Input Perturbation Robustness: Stable to noise")
            except Exception as e:
            ...
            try:
                ...
                print("     ✓ Metamorphic Reasoning: Commutative property preserved")
            except Exception as e:
    ...
    def run_performance_tests(self) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                print("     ✓ CPU Profiling: Multiple reasoning modes tested")
            except Exception as e:
            ...
            try:
                ...
                if result1["output"] == 2 and result2["output"] == 4:
                    ...
                    print("     ✓ Thread Safety: Basic concurrency verified")
                else:
                    performance_results["thread_safety"] = {"status": "FAIL: Incorrect results"}
                    print("     ✗ Thread safety failed: Incorrect arithmetic")
        ...
        for layer_name, layer_results in self.results.items():
            ...
            def count_results(results_dict):
                for key, value in results_dict.items():
                    if key == "error":
                        layer_summary["failed"] += 1  # noqa: B023
                        layer_summary["total"] += 1  # noqa: B023
                    elif isinstance(value, dict):
                        if "status" in value:
                            layer_summary["total"] += 1  # noqa: B023
                            if value["status"] == "PASS":
                                layer_summary["passed"] += 1  # noqa: B023
                            else:
                                layer_summary["failed"] += 1  # noqa: B023
                        else:
                            count_results(value)
