...
def demo_tracing():
    ...
    with tracer.start_span("governance_check", attributes={"action": "api_call"}) as parent:  # noqa: F841
        print("   Started parent span: governance_check")

        time.sleep(0.01)
        tracer.add_span_event("authentication_verified")

        with tracer.start_span("policy_evaluation") as child1:  # noqa: F841
            print("   Started child span: policy_evaluation")
            time.sleep(0.005)
            tracer.add_span_attribute("rules_evaluated", 5)

        with tracer.start_span("violation_detection") as child2:  # noqa: F841
...
def demo_alert_rules():
    ...
    fired = manager.evaluate_rules(test_metrics)  # noqa: F841
