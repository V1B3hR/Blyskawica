...
class AuditRecord(BaseModel):
    ...
    agent_id: str | None = Field(None, description="Agent involved if applicable")
    action: str = Field(..., description="Action that was audited")
    outcome: str = Field(..., description="Outcome of the action")
    risk_score: float | None = Field(None, description="Risk score if applicable")
    metadata: dict[str, Any] = Field(
        default_factory=dict,
        description="Additional metadata",
    )
    merkle_hash: str | None = Field(
        None,
...
async def list_audit_records(
    ...
    period_start: str | None = Query(None, description="Period start (ISO 8601)"),
    period_end: str | None = Query(None, description="Period end (ISO 8601)"),
