...
class EmbeddingEngine:
    ...
    def __init__(
        ...
        config: EmbeddingConfig | None = None,  # noqa: F821
    ):
    ...
    def _create_provider_from_config(self, config: ProviderConfig) -> EmbeddingProvider:  # noqa: F821
    ...
    def _weighted_average(self, embeddings: list[list[float]], weights: list[float]) -> list[float]:
        ...
        for emb, weight in zip(embeddings, weights):  # noqa: B905
            for i in range(min(dim, len(emb))):
