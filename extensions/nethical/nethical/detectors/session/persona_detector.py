...
class PersonaDetector(BaseDetector):
    ...
    async def detect_violations(self, action: AgentAction) -> Sequence[SafetyViolation] | None:
        ...
        if matches >= 3:
            confidence = 0.9
            evidence.append("Multiple persona override attempts detected")
        elif matches >= 2:
            confidence = 0.7
            evidence.append("Persona override patterns detected")
        elif matches >= 1:
            confidence = 0.5
            evidence.append("Potential persona override detected")
        ...
        if confidence >= 0.3:
            ...
            violations.append(SafetyViolation(
                ...
                description="Persona hijack attempt detected",
                evidence=evidence,
