...
class CorrelationMLClassifier:
    ...
    def __init__(self, pattern_threshold: float = 0.5):
        ...
        self.feature_weights: dict[str, float] = dict.fromkeys(self.FEATURE_NAMES, 0.2)
    ...
    def compute_metrics(self, predictions: list[int], labels: list[int]) -> dict[str, float]:
        ...
        tp = sum(1 for p, l in zip(predictions, labels) if p == 1 and l == 1)  # noqa: B905, E741
        tn = sum(1 for p, l in zip(predictions, labels) if p == 0 and l == 0)  # noqa: B905, E741
        fp = sum(1 for p, l in zip(predictions, labels) if p == 1 and l == 0)  # noqa: B905, E741
        fn = sum(1 for p, l in zip(predictions, labels) if p == 0 and l == 1)  # noqa: B905, E741
    ...
    def load(cls, filepath: str) -> "CorrelationMLClassifier":
        ...
        classifier.feature_weights = model_data.get("feature_weights", dict.fromkeys(classifier.FEATURE_NAMES, 0.2))
...
class MultiPatternMLClassifier:
    ...
    def __init__(self, pattern_types=None, pattern_thresholds=None):
        ...
        self.pattern_thresholds = pattern_thresholds or dict.fromkeys(self.pattern_types, 0.5)

        # Feature weights for each pattern_type. Overwritten by training.
        self.feature_weights = {
            # base features as example
            "agent_count": dict.fromkeys(self.pattern_types, 0.2),
            "action_rate": dict.fromkeys(self.pattern_types, 0.2),
            "entropy_variance": dict.fromkeys(self.pattern_types, 0.2),
            "time_correlation": dict.fromkeys(self.pattern_types, 0.2),
            "payload_similarity": dict.fromkeys(self.pattern_types, 0.2),
        ...
        self.trained = dict.fromkeys(self.pattern_types, False)
        self.training_samples = 0
        self.timestamp = None
    ...
    def compute_metrics(preds: dict[str, list[int]], labels: dict[str, list[int]]) -> dict[str, dict[str, float]]:
        ...
        for pt in preds:
            p = preds[pt]
            l = labels[pt]  # noqa: E741
            if len(p) != len(l) or len(p) == 0:
                metrics[pt] = {"accuracy": 0, "precision":0, "recall":0, "f1_score":0, "ece":0}
                continue
            tp = sum(1 for pred, lab in zip(p, l) if pred == 1 and lab == 1)  # noqa: B905
            tn = sum(1 for pred, lab in zip(p, l) if pred == 0 and lab == 0)  # noqa: B905
            fp = sum(1 for pred, lab in zip(p, l) if pred == 1 and lab == 0)  # noqa: B905
            fn = sum(1 for pred, lab in zip(p, l) if pred == 0 and lab == 1)  # noqa: B905
            total = len(p)
            accuracy = (tp + tn) / total if total > 0 else 0.0
            precision = tp / (tp + fp) if (tp + fp) > 0 else 0.0
            recall = tp / (tp + fn) if (tp + fn) > 0 else 0.0
    ...
    def load(cls, filepath: str) -> "MultiPatternMLClassifier":
        ...
        classifier.trained = model_data.get("trained", dict.fromkeys(classifier.pattern_types, False))
        classifier.training_samples = model_data.get("training_samples", 0)
        classifier.timestamp = model_data.get("timestamp")
        return classifier
