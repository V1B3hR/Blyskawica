...
class TestJITOptimizationsPerformance:
    ...
    def test_vectorized_risk_score_performance(self):
        ...
        start = time.time()
        for _ in range(100):
            score = calculate_risk_score_jit(severities, confidences)  # noqa: F841
        elapsed = time.time() - start
...
class TestSystemLimitsDetectorPerformance:
    ...
    def test_monotonic_increase_detection(self):
        ...
        result = all(a < b for a, b in zip(recent_sizes, recent_sizes[1:]))  # noqa: B905
        assert result is True

        # Test with non-monotonic
        non_monotonic = [100, 200, 150, 400, 500]
        result = all(a < b for a, b in zip(non_monotonic, non_monotonic[1:]))  # noqa: B905
