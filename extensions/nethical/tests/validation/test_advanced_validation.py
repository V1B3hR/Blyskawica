...
class FailureInjector:
    ...
    def maybe_inject_failure(self) -> FailureType | None:
...
class AdvancedLoadTestRunner:
    ...
    def __init__(
        ...
        self.results: ValidationResults | None = None
    ...
    def run_scaling_test(
        ...
        try:
            ...
            ramp_start = time.time()  # noqa: F841
        ...
        try:
            while time.time() < end_time:
                ...
                if time.time() - last_progress > 30:
                    ...
                    remaining = end_time - time.time()  # noqa: F841
                    completed = sum(1 for f in futures if f.done())
                    process = psutil.Process()
                    memory_mb = process.memory_info().rss / 1024 / 1024
...
async def test_generate_comprehensive_report(test_runner, output_dir):
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    with open(summary_file, 'w') as f:  # noqa: ASYNC230
