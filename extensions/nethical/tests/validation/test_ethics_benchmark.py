...
class EthicsMetrics:
    ...
    def calculate_metrics(predictions: list[bool], labels: list[bool]) -> dict[str, float]:
        ...
        true_positives = sum(1 for p, l in zip(predictions, labels) if p and l)  # noqa: B905, E741
        false_positives = sum(1 for p, l in zip(predictions, labels) if p and not l)  # noqa: B905, E741
        false_negatives = sum(1 for p, l in zip(predictions, labels) if not p and l)  # noqa: B905, E741
        true_negatives = sum(1 for p, l in zip(predictions, labels) if not p and not l)  # noqa: B905, E741
...
def test_generate_ethics_report(governance, dataset, metrics_calculator, tmp_path):
    ...
    # Test each category
    for category in dataset.categories.keys():  # noqa: SIM118
        samples = dataset.get_category_samples(category)
        predictions = []
        labels = []
    ...
    for category, action_text, is_violation in all_samples:  # noqa: B007
        result = governance.process_action(
            agent_id="benchmark_agent",
            action=action_text
        )
