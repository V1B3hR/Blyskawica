...
def test_explanation_latency_sla(governance, validator):
    ...
    latencies_ms = [l * 1000 for l in result["latencies"]]  # noqa: E741
    ...
    slow_requests = [l for l in latencies_ms if l > 500]  # noqa: E741
    if slow_requests:
    ...
    if avg_latency_ms >= 500 or p95_latency >= 500:
        ...
        logger.error("=" * 80)
        logger.error("Explanation latency exceeds 500ms SLA")
...
def test_generate_explainability_report(governance, validator, tmp_path):
    ...
    latencies_ms = [l * 1000 for l in coverage_result["latencies"]]  # noqa: E741
