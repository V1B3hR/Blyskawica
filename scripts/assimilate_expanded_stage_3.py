...
def perform_expanded_assimilation_stage_3(target_mastery=0.75):
    """
    Expanded Omniscient Assimilation - ETAP 3 (75% Advanced)
    ...
    for step in range(max_steps):
        ...
        if node.phase == "sleep":
            node.step_phase()
            if node.phase != "sleep":
                logger.info(f"🌙 [STRATEGIC_DREAM] Stage 3 Consolidation step {step}. Knowledge hierarchies reorganizing.")
